#!/bin/bash
# ==============================================================================
# MemoryVLA 在 WR03 (LeRobot v3, 13-dim actions) 上的 LoRA 微调启动脚本
# ------------------------------------------------------------------------------
# 决策(与 WA01 一致):
#   * 仅 LLM 注入 LoRA (Llama-2-7B q/k/v/o_proj, r=16, alpha=32, dropout=0.1)
#   * 纯视觉语言输入 (不用 observation.state, 保持 CogACT 架构)
#   * 动作头按 13 维重建 (WR03 mobile manipulator, 同 WA01 动作空间)
#   * 冻结: vision + Llama base (stage="align")
#   * 训练: LoRA adapters + projector + perception memory + DiT-L diffusion head
# ------------------------------------------------------------------------------
# 硬件: 3x A100 80GB | 环境: memvla (torch 2.2 / tf 2.15 / py3.10)
#
# 用法:
#   bash script/train/wr03/train_wr03.sh             # 正式训练 (53333 步, 有效 batch 48)
#   SMOKE_TEST=1 bash script/train/wr03/train_wr03.sh # 冒烟测试 (5 步)
#   RESUME=1 bash script/train/wr03/train_wr03.sh    # 从最新 checkpoint 续训
...
# --- 定位仓库根目录 (脚本位于 script/train/wr03/ 下) ---------------------------
...
# --- 关键路径校验 --------------------------------------------------------------
DATA_DIR="/data1/shared_workspace/chensihang/dataset/membench/wr03_200seeds_v061"
...
if [[ "$RESUME" == "1" ]]; then
    ...
    CKPT="${RESUME_CKPT:-$(ls -t runs/memvla_wr03/checkpoints/*.pt 2>/dev/null | head -1)}"
    [[ -n "$CKPT" && -e "$CKPT" ]] || {
        echo "ERROR: 未找到可续训的 checkpoint (请用 RESUME_CKPT=<path> 或确认 runs/memvla_wr03/checkpoints/*.pt)"
        exit 1
    }
...
bs=8
# 梯度累积 (默认 2): 有效 batch = bs x n_gpu x grad_accum. 3 卡 => 8x3x2 = 48/步.
...
if [[ "${SMOKE_TEST:-0}" == "1" ]]; then
    ...
    run_id="memvla_wr03--smoke"
else
    echo ">>> [FULL RUN] 正式训练 (梯度累积 ${GRAD_ACCUM})"
    # 3 卡 (有效 batch 48): max_steps = 80000 x 32/48 ≈ 53333, 保持总数据量(256万样本)不变
...
TRAIN_ARGS=(
  ...
  --vla.type prism-dinosiglip-224px+wr03+diffusion
